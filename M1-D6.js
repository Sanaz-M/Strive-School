...
let mySkills = [
  { programmingLanguages: ["HTML5", "CSS", "JavaScript"] },
  { graphicDesign: ["Photoshop", "Illustrator", "AfterEffect"] },
...
console.log(whoIsBigger(6, 14) + " " + "is bigger than the other number");
...
let deleteOne= function(s , b){
  if (b === true) return s.slice(1)
...
console.log(onlyLetters("I met her 3 times"));
...
let isThisAnEmail = function(text) {
  let mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
  if (text.value.match(mailformat)) return true;
  else return false;
}
...
let date1 = new Date("07/22/2021");
let date2 = new Date("09/27/2021");

let howManyTimes = date2.getTime() - date1.getTime();
let howManyDays = howManyTimes / (1000 * 3600 * 24);
...
let isTodayMyBirthday = function (newDate) {
  let myBirthday = "02/12/1990";
  if (newDate === myBirthday) return true;
  else return false;
};
